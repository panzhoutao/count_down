...
class FlatScrollWheel extends StatefulWidget {
  ...
  ///
  /// [selectedIndex] Selected index of a specific [FlatScrollWheel]'s countdown_detail.
  ///
  ...
  /// Selected index of a specific [FlatScrollWheel]'s countdown_detail.
  ...
  /// Add an offset to the start of the countdown_detail selection list.
  final int? startOffset;

  /// Add an offset to the end of the countdown_detail selection list.
...
class _FlatScrollWheelState extends State<FlatScrollWheel> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // If `listenAfterAnimation` is enabled, listen when select countdown_detail is changed.
      if (widget.listenAfterAnimation) {
        _controller.position.isScrollingNotifier.addListener(_handleOnSelectedItemChanged);
      }

      // If `startOffset` is specified, listen on countdown_detail changed and animate towards the nearest countdown_detail that is not part of the offset items.
      if (widget.startOffset != null) {
        _controller.position.isScrollingNotifier.addListener(() => _handleOffset(_startOffsets));
      }

      // If `lastOffset` is specified, listen on countdown_detail changed and animate towards the nearest countdown_detail that is not part of the offset items.
      if (widget.lastOffset != null) {
  ...
  void didUpdateWidget(covariant FlatScrollWheel oldWidget) {
    ...
    // Jump to an countdown_detail if the `selectedIndex` is changed or the `items lenght` is changed.
  ...
  /// Handles the boundary of the selection when countdown_detail(s) are part of the `startOffsets` or `lastOffsets`.
  void _handleOffset(List<int> offsets) async {
    ...
    // Check whether the current selected countdown_detail is in the `startOffsets` or `lastOffsets`. Return the value if its true and -1 if not.
    ...
    if (value != -1) {
      ...
      // To put it simply, we are doing this to animate towards the nearest countdown_detail that is outside of the offset list.
      final addOffset = index >= halfSize ? offsets.length - index : -(index + 1);

      // Using the `addOffset`, animate towards the nearest countdown_detail by the adding the current selected countdown_detail index with the `addOffset` value.
  ...
  TextStyle? _handleTextStyle(int itemIndex) {
    // If `_startOffsets` or `_lastOffsets` contains the current `itemIndex`, then mute the countdown_detail. Otherwise, use the default text style.
